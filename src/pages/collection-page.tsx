...
export const CollectionPage = () => {
  ...
  } = useQuery<CollectionAndCardsQueryData, CollectionAndCardsQueryVariables>(
    GET_COLLECTION_AND_CARDS,
    {
      variables: { id },
    }
  );

  const [getCards, { data: cardsData, loading: loadingCards }] = useLazyQuery<
    CardsQueryData,
    CardsQueryVariables
  >(GET_CARDS);
  ...
  return (
    <Container>
      ...
      {errorCollectionAndCards && (
        <Grid item>
          <ReceivedError />
        </Grid>
      )}

      {collectionAndCardsData && !loadingCollectionAndCards && (
        <CollectionDescription
          description={collectionAndCardsData.cardCollection}
        />
      )}
      ...
      {collectionAndCardsData && (
        <CardsPagination
          count={Math.ceil(
            collectionAndCardsData?.cardCollection.cardsCount / 8
          )}
